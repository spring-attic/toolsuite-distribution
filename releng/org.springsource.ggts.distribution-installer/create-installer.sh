...
processTar() {
	...
	if [ ! -d "$2" ]; then
		...
		mkdir $2
		cd $2
		tar zxf ../../../org.springsource.ggts.distribution-bundles/target/$1.gz
		
		if [[ $2 =~ .*linux.* ]]; then
			JDKPLACEHOLDER=`grep JDKPath ./$GGTS_ROOTFOLDER/$STS_DIRECTORY-$STS_VERSION/$STS_APPNAME.ini`
			if [ -z $JDKPLACEHOLDER ] ; then
				echo "-vm" >> ./temp_STS.ini
				echo "\$JDKPath/bin/java" >> ./temp_STS.ini
				cat ./temp_STS.ini ./$GGTS_ROOTFOLDER/$STS_DIRECTORY-$STS_VERSION/$STS_APPNAME.ini >> ./$GGTS_ROOTFOLDER/$STS_DIRECTORY-$STS_VERSION/temp_STS.ini
				rm -rf ./temp_STS.ini
				mv ./$GGTS_ROOTFOLDER/$STS_DIRECTORY-$STS_VERSION/temp_STS.ini ./$GGTS_ROOTFOLDER/$STS_DIRECTORY-$STS_VERSION/$STS_APPNAME.ini
			fi
		fi
...
processZip() {
	...
	if [ ! -d "$2" ]; then
		...
		mkdir $2
		cd $2
		unzip -q ../../../org.springsource.ggts.distribution-bundles/target/$1
...
processZip groovy-grails-tool-suite-$RELEASE_VERSION-$ECLIPSE_VERSION-win32.zip win32 
...
ZIP_NAME=`ls -a macosx.cocoa/$GGTS_ROOTFOLDER/vfabric-tc-server-developer-$TCS_VERSION/ | grep tomcat-6`
TOMCAT6_VERSION=`expr "$ZIP_NAME" : '.*\([0-9]\.[0-9]*\.[0-9]*\.[-,A-Z,a-z,0-9,.]*\).*'`

ZIP_NAME=`ls -a macosx.cocoa/$GGTS_ROOTFOLDER/vfabric-tc-server-developer-$TCS_VERSION/ | grep tomcat-7`
...
#ZIP_NAME=`ls -a macosx.cocoa/$GGTS_ROOTFOLDER/ | grep maven`
